...
def index(request):
    ...
    complete_items = GroceryItem.objects.filter(
        is_completed=True).order_by('-date_completed')
    form = AddForm()
    try:
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            GroceryItem.objects.create(
                name=form.cleaned_data['name'], date_created=timezone.now())
            return HttpResponseRedirect(reverse('glist:index'))
        else:
            return HttpResponseRedirect(f'{reverse("glist:index")}?error=Please enter a name for your new item')
    else:
        raise Http404()
