...
# function to see if all cities have been visited .... unused?
def all_cities_visited(min_tree):
    ...
    for city in min_tree:
        if min_tree[city][1] == True:
            flag = False
    return flag

# function for getting lengths between two cities
def get_shortest_paths(cities, start, input_range):
    ...
    # 1. Mark your selected initial node with a current distance of 0 and the rest with infinity.
    # min_tree will be list used to hold shortest paths
    ...
    while not all_cities_visited(min_tree):
        ...
        for neighbour in cities[current_node]:
            # ignore visited nodes and neighbor noods that aren't in range
            if neighbour in cities_in_range and min_tree[neighbour][1] == False:

                # 3. For each neighbour N of your current node C: add the current distance of C with the weight of the edge connecting C-N. 
                # If it's smaller than the current distance of N, set it as the new current distance of N.
                # trial sum is distance from start node to current node + distance to neighbor
                trial_sum = min_tree[current_node][0] + cities[current_node][neighbour]
                # print(f"Checking neightbor {neighbour}.  Neighbor is {cities[current_node][neighbour]} away from {current_node}, and {trial_sum} away from start: {start}")
                if min_tree[neighbour][0] > trial_sum:
                    # print(F"Neighbor was previously {min_tree[neighbour][0]} far away from {start}.  But since {trial_sum} is less than this, updating.")
                    min_tree[neighbour][0] = trial_sum

        # get next current node
        closest_city_name, closest_city_distance = get_closest_neighbor(cities, min_tree, current_node)
        ...
        # 5. If there are non-visited nodes, go to step 2.
        if closest_city_name == 'No unvisited Cities':
            break
    ...
    # translate min_tree into tuple to be returned
    ret_list = []
    for city in min_tree:
...
while True:
    ...
    if user_input in city_to_accessible_cities_with_travel_time.keys():
        # print cities within one and two hop of input cit
        print_one_and_two_hop_cities(city_to_accessible_cities_with_travel_time, user_input)
...
while True:
    ...
    if user_origin in city_to_accessible_cities_with_travel_time.keys():
        ...
        if user_range > 0:
            ...
            shortest_paths = get_shortest_paths(city_to_accessible_cities_with_travel_time, user_origin, user_range)
