...
function gamble() {
    let money = 0;
    let attempts = 0;
    let attemptTimesRaw = document.getElementById("bets");
    let attemptTimes = parseInt(attemptTimesRaw.value);
    let makebet = document.getElementById("makebet");
    let inthered = document.getElementById("cash-return");
    let i = 0;
    while (i < attemptTimes) {
        money -= 2;
        i++;
    ...
    console.log(attemptTimes);
    return inthered.innerText = `Your financial return was $${money}.`;
}

makebet.addEventListener('click', gamble);