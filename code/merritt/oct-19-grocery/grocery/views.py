...
def index(request):
    ...
    completed_items = GroceryItem.objects.filter(is_completed=True).order_by('-date_completed')
    form = AddForm()
    try:
    ...
    context = {
        ...
        'complete_items': completed_items,
        'error': error,
        'form': form,
    }
...
def add_item(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(f"{reverse('grocery:index')}?error=Please enter a name for your glorious patriotic foodstuff")
